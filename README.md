...
- Smart voice navigation with Android `SpeechRecognizer`: tap the microphone and say a route number (for example “6” or “350”) to scan active live bus markers, animate/zoom the map to the matching bus, or show a route-not-active message.
...
- Location-Based AR Bus Stop Finder mode backed by the live camera preview, smoothed GPS, low-pass-filtered compass/accelerometer data, lerped stop pins, and a glowing pavement path to the selected stop or bus.
