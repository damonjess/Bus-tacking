...
- Location-Based AR Bus Stop Finder mode backed by the live camera preview, rotation-vector sensor fusion, true-north magnetic declination correction, a GPS accuracy lock with a 5-second **Bypass Calibration** fallback, smoothed updates, Google-Live-View-style glowing route ribbon, and high-contrast bus billboards with destination, ETA, and occupancy.
- Optional BODS SIRI-VM XML/JSON polling that refreshes bus locations every 15 seconds while the map is active, colors bus markers by occupancy/crowding (green Easy Seating, yellow Standing Room Only, red Full/Crowded, blue Information Unknown), and stops polling when the map closes to save battery.
